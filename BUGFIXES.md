...
### Variable Expansion Tests (Previously Failing):

- ✅ **Test 186:** `echo "$42"` → now outputs `2` (was empty)
- ✅ **Test 189:** `echo "$!HELLO"` → now outputs `HELLO` (was `!HELLO`)
- ✅ **Test 190:** `echo "$!$USER"` → now outputs `daniel` (was `!daniel`)
- ✅ **Test 191:** `echo "$!$USER!"` → now outputs `daniel!` (was `!daniel!`)
- ✅ **Test 194:** `echo $42` → now outputs `2` (was empty)
- ✅ **Test 195:** `echo $!` → now outputs empty (was `!`)
- ✅ **Test 202:** `echo Hello"my friend$S$USER$S$!'from' 42?"'are you ok?'$USER` → now outputs correct result
- ✅ **Test 203:** `echo Hello"my friend$S$USER$S$!='from' 42?"'are you ok?'$USER` → now outputs correct result
- ✅ **Test 204:** `echo Hello"my friend$S$USER$S$='from' 42?"'are you ok?'$USER` → now outputs `$=` literally
- ✅ **Test 206-210:** All similar tests with `$=` now output the literal `$=` as expected

### Environment Variable Test:

- ✅ **Test 265:** `export | sort | grep -v SHLVL | grep -v "declare -x _" | grep -v "PS.="` → `MANPAGER` now displays correctly
...
The new implementation follows bash's variable expansion rules:

1. **Positional Parameters (`$0`, `$1`, etc.):** Expand to empty in minishell context
2. **Special Parameters (`$!`, `$#`, etc.):** Expand to empty in minishell context
3. **Invalid Names (`$=`, `$+`, etc.):** Treated as literal `$` followed by the character
4. **Valid Variables:** Normal environment variable lookup

### Environment Variable Handling

The fix ensures that:

- Only strings that actually start with quote characters have quotes trimmed
- Environment variables with quotes inside (but not at the start) are preserved
- The `MANPAGER="sh -c 'col -bx | batcat -l man -p'"` example is handled correctly

---

## Files Modified

1. `src/parsing/expand_utils.c` - Variable expansion logic
2. `src/utils/utils.c` - Quote detection function
3. `src/built_ins/export/utils_list_export.c` - Environment variable creation

## Compatibility

These changes ensure bash-compatible behavior while maintaining the existing functionality for normal environment variables and standard variable expansion cases.
